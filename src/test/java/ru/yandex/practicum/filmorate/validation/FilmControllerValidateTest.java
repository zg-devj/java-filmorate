...
public class FilmControllerValidateTest {
    ...
    public void testValidateUser_BlankName() {
        ...
        Film film = new Film(null, " ", "description", now, 100, null, null, null, null);
    ...
    public void testValidateUser_DescriptionMoreThen200Chars() {
        ...
        Film film = new Film(null, "name", description, now, 100, null, null, null, null);

        Set<ConstraintViolation<Film>> violations = validator.validate(film);
        Assertions.assertEquals(1, violations.size());
        Assertions.assertEquals("Длина описания не должна быть больше 200 символов.", violations.iterator().next().getMessage());
    }

    @Test
    public void testValidateUser_ReleaseDateLessThen1895_12_28() {
        LocalDate lessDate = LocalDate.of(1895, 12, 28).minusDays(1);
        Film film = new Film(null, "name", "description", lessDate, 100, null, null, null, null);

        Set<ConstraintViolation<Film>> violations = validator.validate(film);
        Assertions.assertEquals(1, violations.size());
        Assertions.assertEquals("Дата релиза не может быть раньше 1895-12-28", violations.iterator().next().getMessage());
    }

    @Test
    public void testValidateUser_PositiveDuration() {
        LocalDate now = LocalDate.now().minusYears(1);
        Film film = new Film(null, "name", "description", now, -1, null, null, null, null);
